...
def process_singlecmds(singlecmds, G):
    ...
    if 2 * G.pmlthickness['x0'] >= G.nx or 2 * G.pmlthickness['y0'] >= G.ny or 2 * G.pmlthickness['z0'] >= G.nz or 2 * G.pmlthickness['xmax'] >= G.nx or 2 * G.pmlthickness['ymax'] >= G.ny or 2 * G.pmlthickness['zmax'] >= G.nz:
        raise CmdInputError(cmd + ' has too many cells for the domain size x0:{} y0{} z0{}'.format(G.pmlthickness['x0'], G.pmlthickness['y0'], G.pmlthickness['z0']))
    ...
    if singlecmds[cmd] is not None:
        ...
        for waveform in range(len(waveformIDs)):
            ...
            w.userfunc = [waveformtime, singlewaveformvalues]


            if G.messages:
                # print('User waveform and func {} {} created using {} and, if required, interpolation parameters (kind: {}, fill value: {}).'.format(w.userfunc, w.ID, timestr, 'nearest', kwargs['fill_value']))
                print('------------User defined waveform being used without interpolation-----------------')
