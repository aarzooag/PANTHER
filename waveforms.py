...
class Waveform(object):
    ...
    def calculate_value(self, time, dt):
        ...
        if self.type == 'gaussian':
            ...
            ## NO INTERPOLATION FOR USER DEFINED WAVEFORM
            # ampvalue = self.userfunc(time)
            # print('looking for time:{}'.format(time))
            # print('times{}'.format(list(self.userfunc[0])))
            # t_idx = np.where(self.userfunc[0] == time)
            t_idx = int(time/dt)
            ampvalue = self.userfunc[1][t_idx]
